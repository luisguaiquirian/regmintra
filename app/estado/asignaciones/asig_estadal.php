                  ...
                  <tbody id="cont_tabla_entrada">
                    <?php
                      ...
                      foreach ($system->sql() as $rs) {
                        echo '<tr id =fls_'.$x.'>
                              <td>'.$rs->estado.'</td>
                              <td><a data-tool="tooltip" title="Listar asignacion por lineas de transporte del municipio '.$rs->municipio.' estado '.$rs->estado.'" href="'.$_SESSION['base_url1'].'app/estado/asignaciones/asig_municipal.php?e='.base64_encode($rs->estado).'&ide='.base64_encode($rs->id_estado).'&m='.base64_encode($rs->municipio).'&idm='.base64_encode($rs->id_municipio).'&r='.base64_encode($tipo).'&i='.base64_encode($rs->subtipo).'&d='.base64_encode($descripcion).'">'.$rs->municipio.'<a></td>
