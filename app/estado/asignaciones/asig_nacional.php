    ...
    switch ($tipo) {
      case '1'://neumatico
        $system->sql="SELECT e.estado,e.id_estado,c.id as id_producto,c.neumatico as subtipo,
          ...
          where b.id_rubro=".$tipo." and c.neumatico='".$subtipo."' and (a.estatus=1 and b.estatus=1) and a.estado = ".$_SESSION['edo']."
          GROUP BY a.estado";
      break;
      case '2'://Lubricante
        $system->sql="SELECT e.estado,e.id_estado,l.id as id_producto,l.lubricante as subtipo,
          ...
          where b.id_rubro=".$tipo." and l.lubricante='".$subtipo."' and (a.estatus=1 and b.estatus=1) and a.estado = ".$_SESSION['edo']."
          GROUP BY(a.estado)";
      break;
      case '3'://Acumulador
        $system->sql="SELECT e.estado,e.id_estado,d.id as id_producto,d.acumulador as subtipo,
          ...
          where b.id_rubro=".$tipo." and d.acumulador='".$subtipo."' and (a.estatus=1 and b.estatus=1) and a.estado = ".$_SESSION['edo']."
          GROUP BY(a.estado)";
      break;
      default:
            ...
            <tbody id="cont_tabla_entrada">
              <?php
                ...
                foreach ($system->sql() as $rs) {
                  echo '<tr id =fls_'.$x.'>
                        <td><a data-tool="tooltip" title="Listar asignacion por municipio del estado '.$rs->estado.'" href="'.$_SESSION['base_url1'].'app/estado/asignaciones/asig_estadal.php?e='.base64_encode($rs->estado).'&ide='.base64_encode($rs->id_estado).'&d='.base64_encode($descripcion).'&r='.base64_encode($tipo).'&i='.base64_encode($subtipo).'">'.$rs->estado.'<a></td>
