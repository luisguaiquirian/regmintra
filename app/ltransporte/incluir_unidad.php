...
if(isset($_GET['agregar']))
	{
/*        $system->table = "rutas";
        $system->where = "estado =$_SESSION[edo] AND municipio =$_SESSION[mun] AND rutas.cod_linea=$_SESSION[user]";
        $rut = $system->count();
        if($rut < 0)
        {
    ...
    if(isset($_GET['modificar_unidad']))
	{
		// si existe el where de modificar buscamos el registrp
		$system->table = "unidades";

		$register = $system->find(base64_decode($_GET['modificar_unidad']));
		
        $system->table = "users";
		$regist = $system->find(base64_decode($_GET['id_usuario']));		
	}
/*        $system->table = "rutas";
        $system->where = "estado =$_SESSION[edo] AND municipio =$_SESSION[mun] AND rutas.cod_linea=$_SESSION[user]";
        $rut = $system->count();
...
?>
	...
	<section class="panel">
		...
		<div class="panel-body">
			<form name="form_registrar"  id="form_registrar" action="./operaciones.php" class="form-horizontal" method="POST" enctype="multipart/form-data">
				...
				<input type="hidden" id="action" name="action" value="<?= $register ? 'modificar_unidad' : 'grabar_unidad' ?>">
				...
				<fieldset>
					...
					<select name="tipo_unidad" id="tipo_unidad" required="" class="form-control">
                      ...
                      <div class="col-md-4">                     
                        <input class="form-control" type="text" id="cap" placeholder="Cantidad de Puestos" maxlength="12" name="cap" onkeypress="return valida(event)" value="<?= $register ? $register->cap : '' ?>">
                      </div>                    
                    </div>
					<div class="form-group">              
					...
					<select name="modelo" id="modelo" required="" class="form-control">
 					...
 					<div class="form-group">
                      ...
                      <div class="col-md-4">                     
                        <input class="form-control" type="text" id="ano" placeholder="Año de fabricación del Vehículo" name="ano" onkeypress="return valida(event)" value="<?= $register ? $register->ano : '' ?>">
                      </div>
 					...
 					<div class="form-group">
                      <label class="col-md-2 col-sm-2 control-label" for="color">Color:</label>
                      <div class="col-md-4">                     
                        <input class="form-control" onKeyUp="this.value=this.value.toUpperCase();" type="text" id="color" placeholder="Color del Vehículo" name="color" value="<?= $register ? $register->color : '' ?>">
                      </div>
<!--                      <label class="col-md-2 col-sm-2 control-label" for="foto">Foto:</label>
                      <div class="col-md-4">                     
						...
						<option value=""></option>
						<?
							$system->sql = "SELECT * from rutas where estado =".$_SESSION['edo']." AND municipio =".$_SESSION['mun'];
							foreach ($system->sql() as $rs) 
							{
					...
					<select name="tipo_lub" id="tipo_lub" required="" class="form-control">
                      ...
                      <div class="col-md-4">                     
                        <input class="form-control" type="text" id="cant_lubri" placeholder="Litros de Lubricante que usa" maxlength="12" name="cant_lubri" onkeypress="return valida(event)" value="<?= $register ? $register->cant_lubri : '' ?>">
                      </div> 
                   </div>                      
                   <div class="form-group">
                      <label class="col-md-2 col-sm-2 control-label" for="num_neu">Número de neumático:</label>
                      <div class="col-md-4">                     
					<select name="num_neu" id="num_neu" required="" class="form-control">
						<option value=""></option>
						<?
							$system->sql = "SELECT * from cauchos";
							foreach ($system->sql() as $rs) 
							{

								if($register->num_neu == $rs->id)
								{

									echo '<option value="'.$rs->id.'" selected>'.$rs->neumatico.'</option>';	
								}
								else
								{
									echo '<option value="'.$rs->id.'">'.$rs->neumatico.'</option>';	
								}
                      ...
                      <div class="col-md-4">                     
                        <input class="form-control" type="text" id="cant_neu" placeholder="Total de Neumáticos que usa la unidad" maxlength="2" name="cant_neu" onkeypress="return valida(event)" value="<?= $register ? $register->cant_neu : '' ?>">
                      </div> 
                   </div> 
                     <div class="form-group">
                      <label class="col-md-2 col-sm-2 control-label" for="acumulador">Acumulador:</label>
						<div class="col-md-4 col-sm-4">
					<select name="acumulador" id="acumulador" required="" class="form-control">
						<option value=""></option>
						<?
							$system->sql = "SELECT * from acumuladores";
							foreach ($system->sql() as $rs) 
							{

								if($register->acumulador == $rs->id)
								{

									echo '<option value="'.$rs->id.'" selected>'.$rs->acumulador.'</option>';	
								}
								else
								{
									echo '<option value="'.$rs->id.'">'.$rs->acumulador.'</option>';	
								}
                   ...
                   <div class="form-group">                 
                     <label class="col-md-2 col-sm-2 control-label" for="obser">Observaciones:</label>
                      <div class="col-md-10">                     
