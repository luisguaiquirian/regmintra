    ...
    case 'asignar_items_estado':
      ...
      if($r['r'] == true){
        ...
        for ($i=0; $i < $cant_alm; $i++) { 
          ...
          $r = $system->guardar_begin(array('id_mov'=>$idnew,'id_almacen'=>$almacenes[$i][0],'cantidad'=>$almacenes[$i][1],'estatus'=>1),$conexion); 
          if ($r['r']==true) {
            $system->sql = "SELECT disponible,comprometido FROM inventario WHERE id = ".$almacenes[$i][0];
            $sel = $system->sql();
            if(count($sel)>0){
              if ($sel[0]->disponible>=$almacenes[$i][1]) {
                $dis = $sel[0]->disponible-$almacenes[$i][1];
                $com = $sel[0]->comprometido+$almacenes[$i][1];
      ...
      if ($pas == 0) {
       ...
       echo json_encode(array('msg' => false, 'msj'=>'El movimiento de asignacion de items de estado se realizo exitosamente.')); 
      }
    break;
      ...
      switch ($item) {
        case '1'://neumatico
          $system->sql="select b.descripcion,b.id as rubro,c.neumatico as item,count(*) as solicitudes,SUM(a.cantidad) as cantidad  
            ...
            where b.id=".$item." and a.estatus=1
